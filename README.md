...
> Note: You **must** use `NewBreaker` to create circuit breakers.  
...
> Take care when using the `OpeningWillResetErrors` flag.  If used in conjunction
...
If the interpolation function returns 100, all calls to `Run` return `circuit.StateThrottledError`.
If the interpolation function returns 0, no calls to `Run` return `circuit.StateThrottledError`.
